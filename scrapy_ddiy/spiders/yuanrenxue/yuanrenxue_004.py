...
def get_j_key(data_key: str, data_value: str):
    """获取用于判断哪些图片会被隐藏的j_key"""
    return subprocess.check_output(
        ['node', 'scrapy_ddiy/scripts/js/yuanrenxue/004.js', data_key, data_value]).decode().strip()


class Yuanrenxue004(DdiyBaseSpider):
    name = 'yuanrenxue_004'
    description = '猿人学刷题平台-第4题雪碧图样式干扰'
    # 数字总和
    number_sum = 0

    def __init__(self, name=None, **kwargs):
        super().__init__(name, **kwargs)
        self.search_style_left = re.compile('left:([-\\d.]+?)px')

    def start_requests(self):
        headers = {'User-Agent': 'yuanrenxue.project'}
        for i in range(1, 6):
            url = f'http://match.yuanrenxue.com/api/match/4?page={i}'
            yield Request(url=url, headers=headers, callback=self.parse, meta={'current_page': i})
    ...
    def parse(self, response, **kwargs):
        ...
        j_key = get_j_key(data_key, data_value)
        sel = Selector(text=data['info'], type='html')
        # 本页数字的列表
        final_num_li = []
        for td in sel.xpath('//td'):
            # 临时存储数字的列表，预设长度为 10
            img_num_li = [''] * 10
            for index, img in enumerate(td.xpath(f'./img[not(@class="img_number {j_key}")]')):
                style_left_px = img.xpath('./@style').get()
                b64_img = img.xpath('./@src').get()
                style_left_px = self.search_style_left.search(style_left_px).group(1)
                offset = int(float(style_left_px) / 11.5)
                img_num_li[index + offset] = B64_IMG_MAP_NUM[b64_img]
            final_num = ''.join(img_num_li)
            final_num_li.append(int(final_num))
        self.logger.info(f'第 {current_page} 页数字为：{final_num_li}')
        self.number_sum += sum(final_num_li)

    def closed(self, reason):
        super().closed(reason)
        self.logger.info(f'数字总和为：{self.number_sum}')
